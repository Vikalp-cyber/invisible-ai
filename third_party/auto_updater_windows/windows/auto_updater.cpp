...
class AutoUpdater {
 public:
  ...
  void AutoUpdater::DeliverWinSparkleEvent(const std::string& eventName);
  void AutoUpdater::SetTargetHwnd(HWND hwnd);
...
void AutoUpdater::DeliverWinSparkleEvent(const std::string& eventName) {
  ...
  args[flutter::EncodableValue("type")] = flutter::EncodableValue(eventName);
...
void AutoUpdater::OnWinSparkleEvent(std::string eventName) {
  ...
  if (target_hwnd_ == nullptr) {
    DeliverWinSparkleEvent(eventName);
    return;
  }
  auto* heap = new std::string(std::move(eventName));
  if (!PostMessageW(target_hwnd_, FLUTTER_AUTO_UPDATER_WIN_EVENT_MSG,
                    reinterpret_cast<WPARAM>(heap), 0)) {
    DeliverWinSparkleEvent(*heap);
    delete heap;
  }
}
