...
AutoUpdaterWindowsPlugin::AutoUpdaterWindowsPlugin(
  ...
  if (view != nullptr) {
    auto_updater.SetTargetHwnd(view->GetNativeWindow());
  }
...
std::optional<LRESULT> AutoUpdaterWindowsPlugin::OnTopLevelWindowProc(
    HWND hwnd,
    UINT message,
    WPARAM wparam,
    LPARAM lparam) {
  if (message != FLUTTER_AUTO_UPDATER_WIN_EVENT_MSG) {
    return std::nullopt;
  }
  auto* payload = reinterpret_cast<std::string*>(wparam);
  if (payload != nullptr) {
    auto_updater.DeliverWinSparkleEvent(*payload);
    delete payload;
  }
  return 0;
}
